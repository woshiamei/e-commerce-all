...
def raw_data_by_sql():
    ...
    # user_item_action = hdb.get_all("""
    # select t1.fullvisitorid, t1.productsku, t1.p_click, t1.`date`
    # from (
    #     select fullvisitorid, productsku, p_click, `date`
    #         from ods.s_bq_actlist
    #         where `date` > '{}'
    ...
    #         -- , sum(cast(is_addtocart as BIGINT)) + sum(cast(is_order as BIGINT)) as add_order_sum
    #         from ods.s_bq_actlist
    #         where `date` > '{}'
    #         and productsku regexp '^[0-9]+$'
    ...
    #     order by t1.fullvisitorid, t1.`date`
    # """.format(stime, stime, threshold))

    # user_item_action = hdb.get_all(
    # """
    # select t3.fullvisitorid, t3.productsku, t3.p_click, t3.`date`
    # from (
    #     select t1.fullvisitorid, t1.productsku, t1.p_click, t1.`date`
    #     from (
    #         select fullvisitorid, productsku, p_click, `date`
    ...
    #     ) t5
    #  )t6 on t3.productsku = t6.productsku
    # order by t3.fullvisitorid, t3.`date`
    # """.format(stime, stime, threshold))
    ...
    from (
        ...
        from (
            ...
            and productsku regexp '^[0-9]+$'
            GROUP BY productsku
            having sum(cast(p_view as BIGINT)) > 2000
        ) t5
    )t6 on t3.productsku = t6.productsku
    order by t3.fullvisitorid, t3.`date` 
    """.format(stime, stime, threshold))
...
if __name__ == '__main__':
    ...
    # 以 email 主维度， uid 为次维度
    file_raw_data = "../data/raw_sample_swing_click_{}.csv".format(cur_time)
    swing_user_dict_tmp_path = "../data/swing_click_user_dict_tmp_{}.json".format(cur_time)
    swing_item_dict_tmp_path = "../data/swing_click_item_dict_tmp_{}.json".format(cur_time)
    swing_sim_tmp_path = "../data/swing_click_sim_tmp_{}.json".format(cur_time)
    swing_sim_res_path = '../data/swing_click_res_{}.csv'.format(cur_time)
    ...
    with open("/root/reco/i2i/data/swing_done_file.csv", "w") as f:
        f.write("{}".format(swing_sim_res_path) + '\n')
